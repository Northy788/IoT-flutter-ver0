# 1 "/mnt/sdb1/Arduino/Esp8266_test/Esp8266_test.ino"
# 2 "/mnt/sdb1/Arduino/Esp8266_test/Esp8266_test.ino" 2
...
void setup()
{
    ...
    boolean result = WiFi.softAP("ESP8266", "1234567890", 1, 0, 1);
    if (result == true)
    {
    ...
    if (tcp_client)
    {
        if (tcp_client.connected())
        {
            ...
            if (readed && i)
            {
                Serial.printf("\nMessage : %s", buffer);
                data = buffer;
                digitalWrite(LED, (data == "ON") ? 0x1 : 0x0);
            }
        }
        else
        {
